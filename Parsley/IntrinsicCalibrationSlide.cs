...
namespace Parsley {
  public partial class IntrinsicCalibrationSlide : FrameGrabberSlide {
    ...
    private Core.IntrinsicCalibration _ic; //objek penyimpan setting kalibrasi intrinsik, inisialisasi dilakukan ketika pola kalibrasi sudah dipilih
    ...
    protected override void OnSlidingIn(SlickInterface.SlidingEventArgs e) { //dijalankan ketika pertama kali form di-load
      this.Reset();
      base.OnSlidingIn(e);
    }

    protected override void OnSlidingOut(SlickInterface.SlidingEventArgs e) { //dijalankan ketika form ditutup
    ...
    void Reset() 
    {
      if (_ic != null)  //jika terdapat buffer kalibrasi kosongkan
        _ic.ClearViews();
      _first_time = true;
      _timer_auto.Enabled = false;
    ...
    void HandleTakeImageRequest() 
    {
      ...
      if (_take_image_request) 
      {
        if (pattern.PatternFound) 
        {
          ...
          {  
            _btn_calibrate.Enabled = _ic.Views.Count > 2 && !_cb_auto_take.Checked; //jika gambar kalibrasi sudah lebih dr 2 dan tombol auto take sudah di uncheck, enable-kan tombol kalibrasi
          });
        }
      }
